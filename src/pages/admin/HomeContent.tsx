...
export default function HomeContent() {
  ...
  function renderPrimaryField() {
    ...
    return (
      <div>
        <label className="block text-xs font-medium text-slate-500 mb-1">{tab === 'principles' ? 'Principio *' : 'Etiqueta *'}</label>
  ...
  function renderSecondaryFields() {
    return (
      <>
        ...
        {tab !== 'principles' && (
          <div>
            <label className="block text-xs font-medium text-slate-500 mb-1">Icono (lucide-react)</label>
  ...
  return (
    <div className="flex gap-6 max-w-7xl">
      <div className={`${panelOpen ? 'hidden xl:block xl:w-1/2' : 'w-full'} space-y-5`}>
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Contenido del Home</h1>
          <p className="text-slate-500 text-sm mt-1">Administra las listas y tarjetas que aparecen en la página de inicio.</p>
        </div>

        <div className="flex gap-2 flex-wrap">
          {TABS.map((t) => (
            <button key={t.key} onClick={() => { setTab(t.key); closePanel(); }}
              className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${tab === t.key ? 'bg-sky-600 text-white' : 'bg-white text-slate-500 hover:text-slate-900 border border-slate-200'}`}>
              {t.label}
            </button>
          ))}
        </div>
        ...
        <div className="flex items-center justify-between">
          <p className="text-slate-500 text-sm">{items.length} elementos</p>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden">
          {loading ? (
            ...
            <div className="p-12 text-center">
              <p className="text-slate-500 text-sm mb-4">No hay elementos. ¡Agrega el primero!</p>
            ...
            <div className="divide-y divide-slate-100">
              {items.map((item) => (
                <div key={item.id} className={`flex items-center gap-3 px-4 py-3.5 hover:bg-slate-100 transition-colors ${!item.published ? 'opacity-50' : ''}`}>
                  <GripVertical className="w-4 h-4 text-slate-700 flex-shrink-0" />
                  <div className="flex-1 min-w-0">
                    <p className="text-slate-900 text-sm font-medium truncate">{renderItemLabel(item)}</p>
                  ...
                  <button onClick={() => togglePublish(item)} className="p-1.5 text-slate-500 hover:text-white hover:bg-slate-100 rounded transition-colors flex-shrink-0">
                    {item.published ? <Eye className="w-3.5 h-3.5" /> : <EyeOff className="w-3.5 h-3.5" />}
                  </button>
                  <button onClick={() => openEdit(item)} className="p-1.5 text-slate-500 hover:text-white hover:bg-slate-100 rounded transition-colors flex-shrink-0"><Edit2 className="w-3.5 h-3.5" /></button>
                  <button onClick={() => handleDelete(item.id)} className="p-1.5 text-slate-500 hover:text-red-400 hover:bg-red-500/10 rounded transition-colors flex-shrink-0"><Trash2 className="w-3.5 h-3.5" /></button>
                </div>
              ))}
            </div>
          )}
      ...
      {panelOpen && (
        <div className="flex-1 xl:max-w-lg">
          <div className="bg-white border border-slate-200 rounded-2xl p-6 space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="text-slate-900 font-bold">{isNew ? 'Nuevo elemento' : 'Editar elemento'}</h2>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <input type="number" value={(form.sort_order as number) ?? 0} onChange={(e) => setForm((f) => ({ ...f, sort_order: parseInt(e.target.value) || 0 }))}
                    className="w-24 px-3 py-2.5 bg-white border border-slate-200 rounded-xl text-slate-900 text-sm focus:outline-none focus:border-sky-500 transition-colors" />
                </div>
