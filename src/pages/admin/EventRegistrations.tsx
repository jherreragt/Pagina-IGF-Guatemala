    ...
    <div className="space-y-6 max-w-5xl">
      {edition && (
        <div className="flex items-center gap-2 px-4 py-2.5 bg-sky-500/5 border border-sky-500/20 rounded-xl text-xs text-slate-500">
          ...
          Viendo registros de: <span className="text-sky-700 font-semibold">{edition.title}</span>
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Registros de participantes</h1>
          <p className="text-slate-500 text-sm mt-1">{registrations.length} registros totales · {presencialCount} presenciales · {virtualCount} virtuales</p>
        </div>
        <button onClick={exportCSV} className="flex items-center gap-2 px-4 py-2.5 bg-slate-700 hover:bg-slate-100 text-white font-semibold text-sm rounded-xl transition-colors flex-shrink-0">
              ...
              <tbody className="divide-y divide-slate-100">
                {filtered.map((r) => (
                  <tr key={r.id} className="hover:bg-slate-100 transition-colors">
                    ...
                    <td className="px-4 py-3 text-slate-500 text-xs">{r.organization ?? '—'}</td>
