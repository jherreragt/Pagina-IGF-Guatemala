...
import { Plus, Trash2, Edit2, Save, X, CheckCircle2, AlertCircle, Download } from 'lucide-react';
import { supabase, EventResource } from '../../lib/supabase';

const RESOURCE_TYPES = ['PDF', 'ZIP', 'LINK', 'VIDEO', 'PRESENTACIÓN', 'OTRO'];
const EMPTY: Omit<EventResource, 'id' | 'created_at'> = {
...
export default function EventResources() {
  ...
  const [form, setForm] = useState<Omit<EventResource, 'id' | 'created_at'>>(EMPTY);
  ...
  async function load() {
    setLoading(true);
    const { data } = await supabase.from('event_resources').select('*').order('sort_order').order('created_at');
  ...
  async function handleSave() {
    ...
    let result;
    if (isNew) {
      result = await supabase.from('event_resources').insert(form).select().single();
    } else {
      result = await supabase.from('event_resources').update(form).eq('id', editing!.id).select().single();
    }
    setSaving(false);
    ...
    <div className="flex gap-6 max-w-7xl">
      <div className={`${panelOpen ? 'hidden xl:block xl:w-1/2' : 'w-full'} space-y-5`}>
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-white">Recursos del evento</h1>
            <p className="text-slate-400 text-sm mt-1">{resources.length} recursos</p>
          </div>
        ...
        <div className="bg-slate-800/50 border border-white/10 rounded-2xl overflow-hidden">
          {loading ? (
            ...
            <div className="p-12 text-center">
              ...
              <p className="text-slate-400 text-sm mb-4">No hay recursos. ¡Agrega el primero!</p>
            ...
            <div className="divide-y divide-white/5">
              {resources.map((r) => (
                <div key={r.id} className={`flex items-center gap-4 px-4 py-3.5 hover:bg-white/5 transition-colors ${!r.published ? 'opacity-50' : ''}`}>
                  <div className="flex-1 min-w-0">
                    <p className="text-white text-sm font-medium truncate">{r.title}</p>
                    {r.description && <p className="text-slate-500 text-xs mt-0.5 truncate">{r.description}</p>}
                  </div>
                  <span className="text-slate-400 text-xs font-mono flex-shrink-0">{r.resource_type}</span>
                  <div className="flex items-center gap-1 flex-shrink-0">
                    <button onClick={() => openEdit(r)} className="p-1.5 text-slate-400 hover:text-white hover:bg-white/10 rounded transition-colors"><Edit2 className="w-3.5 h-3.5" /></button>
                    <button onClick={() => deleteResource(r.id)} className="p-1.5 text-slate-400 hover:text-red-400 hover:bg-red-500/10 rounded transition-colors"><Trash2 className="w-3.5 h-3.5" /></button>
                  </div>
                </div>
              ))}
            </div>
      ...
      {panelOpen && (
        <div className="flex-1 xl:max-w-sm">
          <div className="bg-slate-800/50 border border-white/10 rounded-2xl p-6 space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="text-white font-bold">{isNew ? 'Nuevo recurso' : 'Editar recurso'}</h2>
            ...
            <div className="space-y-3">
              ...
              <div>
                <label className="block text-xs font-medium text-slate-400 mb-1">URL del archivo / enlace</label>
                <input value={form.file_url ?? ''} onChange={(e) => setForm((f) => ({ ...f, file_url: e.target.value }))} placeholder="https://..."
                  className="w-full px-3 py-2.5 bg-slate-900 border border-white/10 rounded-xl text-white text-sm focus:outline-none focus:border-sky-500 transition-colors" />
              </div>
              <div>
                <label className="block text-xs font-medium text-slate-400 mb-1">Tipo de archivo</label>
