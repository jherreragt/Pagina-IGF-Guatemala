  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Lineamientos de convivencia</h1>
      ...
      <div className="bg-amber-500/10 border border-amber-500/20 rounded-2xl p-5">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            <p className="text-slate-900 font-semibold text-sm">Re-aceptación al actualizar</p>
          ...
          <button
            onClick={bumpVersion}
            disabled={bumping}
            className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-amber-600 hover:bg-amber-500 text-white text-xs font-semibold rounded-lg transition-colors disabled:bg-slate-300 flex-shrink-0"
          >
            {bumping ? (
      ...
      {loading ? (
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-10 text-center">
          ...
          <p className="text-slate-500 text-sm">No hay secciones del código de conducta.</p>
        </div>
      ) : (
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    {!section.is_active && (
                      <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-slate-500/10 text-slate-500">
                        Inactiva
                      </span>
                    )}
                  </div>
                ...
                <div className="flex items-center gap-1 flex-shrink-0">
                  <button
                    ...
                    disabled={i === 0}
                    className="text-slate-500 hover:text-sky-400 disabled:opacity-30 transition-colors w-7 h-7 rounded-lg flex items-center justify-center"
                  >
                  ...
                  <button
                    ...
                    className="text-slate-500 hover:text-sky-400 transition-colors w-7 h-7 rounded-lg flex items-center justify-center"
                  >
                    <Edit2 className="w-4 h-4" />
                  </button>
                  <button
                    onClick={() => handleDelete(section)}
                    className="text-slate-500 hover:text-red-400 transition-colors w-7 h-7 rounded-lg flex items-center justify-center"
                  >
      ...
      {(editing || isNew) && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-lg bg-white border border-slate-200 rounded-2xl shadow-2xl p-6 space-y-4">
            <div className="flex items-center justify-between">
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-3 py-2 bg-white border border-slate-200 rounded-lg text-slate-900 text-sm focus:outline-none focus:border-sky-500"
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-500 mb-1.5">Cuerpo</label>
              <textarea
                value={form.body}
                onChange={(e) => setForm({ ...form, body: e.target.value })}
                rows={5}
                className="w-full px-3 py-2 bg-white border border-slate-200 rounded-lg text-slate-900 text-sm focus:outline-none focus:border-sky-500 resize-none"
              />
            </div>
            ...
            <div className="flex items-center justify-end gap-3 pt-2">
              <button
                onClick={closePanel}
                className="px-4 py-2 text-slate-500 text-sm font-medium hover:text-slate-900 transition-colors"
              >
                Cancelar
              </button>
              <button
                onClick={handleSave}
                disabled={saving}
                className="inline-flex items-center gap-2 px-4 py-2 bg-sky-600 text-white text-sm font-semibold rounded-lg hover:bg-sky-500 disabled:bg-slate-300 transition-colors"
              >
                {saving ? (
