    ...
    <div className="flex gap-6 max-w-7xl">
      <div className={`${panelOpen ? 'hidden xl:block xl:w-1/2' : 'w-full'} space-y-5`}>
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-slate-900">Ponentes y moderadores</h1>
            <p className="text-slate-500 text-sm mt-1">{speakers.length} personas registradas</p>
          </div>
        ...
        <div className="flex gap-2 flex-wrap">
          {categories.map((c) => (
            <button key={c} onClick={() => setFilter(c)}
              className={`px-3 py-1.5 rounded-full text-xs font-medium transition-colors ${filter === c ? 'bg-sky-600 text-white' : 'bg-white text-slate-500 hover:text-slate-900 border border-slate-200'}`}>
              {c}
            </button>
          ))}
        </div>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden">
          {loading ? (
            ...
            <div className="p-12 text-center">
              ...
              <p className="text-slate-500 text-sm mb-4">No hay ponentes. ¡Agrega el primero!</p>
            ...
            <div className="divide-y divide-slate-100">
              {filtered.map((s) => (
                <div key={s.id} className={`flex items-center gap-4 px-4 py-3.5 hover:bg-slate-100 transition-colors ${!s.published ? 'opacity-50' : ''}`}>
                  ...
                  <span className="px-2 py-0.5 bg-sky-500/10 text-sky-700 rounded-full text-xs flex-shrink-0">{s.category}</span>
                  <div className="flex items-center gap-1 flex-shrink-0">
                    <button onClick={() => openEdit(s)} className="p-1.5 text-slate-500 hover:text-white hover:bg-slate-100 rounded transition-colors"><Edit2 className="w-3.5 h-3.5" /></button>
                    <button onClick={() => deleteSpeaker(s.id)} className="p-1.5 text-slate-500 hover:text-red-400 hover:bg-red-500/10 rounded transition-colors"><Trash2 className="w-3.5 h-3.5" /></button>
                  </div>
                </div>
              ))}
            </div>
      ...
      {panelOpen && (
        <div className="flex-1 xl:max-w-lg">
          <div className="bg-white border border-slate-200 rounded-2xl p-6 space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="text-slate-900 font-bold">{isNew ? 'Nuevo ponente' : 'Editar ponente'}</h2>
